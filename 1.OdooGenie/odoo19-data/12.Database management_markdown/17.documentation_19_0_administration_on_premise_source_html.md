...
Use a package manager (Homebrew, MacPorts) to download and install Python 3 if needed.
...
Navigate to the path of the Odoo Community installation (`CommunityPath`) and run **pip** on the requirements file in a terminal **with Administrator privileges** :
...
LinuxWindowsMac OS
  ...
  1. Download and install **nodejs** with a package manager (Homebrew, MacPorts).
