Source: https://www.odoo.com/documentation/19.0/administration/odoo_sh/advanced/containers.html
...
While accessing a container with the shell, you can access the database using _psql_.
...
**Be careful !** Use transactions (_BEGIN…COMMIT/ROLLBACK_) for every _sql_ statements leading to changes (_UPDATE_ , _DELETE_ , _ALTER_ , …), especially for your production database.
...
You can find in the logs (_~/logs/odoo.log_) the addons path used by Odoo.sh to run your server. Look for “ _odoo: addons paths_ ”:
...
Finally, _via_ the Odoo Shell, you can trigger the piece of code/function/method you want to debug.
