Source: https://www.odoo.com/documentation/19.0/applications/sales/sales/invoicing/down_payment.html
...
Doing so reveals another Customer Invoice Draft page, listing _all_ the invoices for that specific sales order in the Invoice Lines tab. Each invoice line item displays all the necessary information related to each invoice.
...
To utilize the Credit Note option, the _Inventory_ application must be installed, in order to confirm the delivery. Otherwise, the delivered quantity can be entered manually directly on the sales order.
...
To change or adjust the income account attached to the Down Payment product page, the _Accounting_ app **must** be installed.
...
With the _Accounting_ app installed, the Accounting tab becomes available on the product page.
