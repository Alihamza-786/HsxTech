Source: https://www.odoo.com/documentation/19.0/applications/hr/lunch/user-accounts.html
...
To add funds and manage user accounts, the user must have Administrator access rights set for the **Lunch** application. This is verified by navigating to Settings app ‣ __ Manage Users. Then, click on a user to view their various settings and access rights.

For more information, refer to the Access rights documentation.
...
Some examples of how money can be organized and transferred within a company:
  ...
  * Money is automatically deducted from the user’s paychecks, then the **Lunch** app manager updates the account when paychecks are issued. This requires adding a salary attachment for the user’s payslip in the _Payroll_ app.
...
An overview of every transaction in the **Lunch** app, including all cash deposits and purchases, can be viewed on the main _Control Accounts_ dashboard. To access this dashboard, navigate to Lunch app ‣ Manager ‣ Control Accounts.
...
The default view is to have all individual transactions hidden. To view all transactions for a user, click the __(triangle) icon to the left of the desired name to expand that specific group.
