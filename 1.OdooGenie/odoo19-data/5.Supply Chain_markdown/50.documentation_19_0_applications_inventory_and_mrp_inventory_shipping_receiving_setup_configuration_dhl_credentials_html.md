Source: https://www.odoo.com/documentation/19.0/applications/inventory_and_mrp/inventory/shipping_receiving/setup_configuration/dhl_credentials.html

# DHL integration

DHL is one of the shipping carriers for which a _shipping connector_ is available in Odoo’s **Inventory** app. By enabling the shipping connector in the app’s settings, and configuring at least one _shipping method_ , the process of calculating shipping rates and generating shipping labels is greatly simplified.
...
Then, click the __ DHL Shipping Methods link to open a page showing all shipping methods with the _Provider_ set to DHL.
...
The _SiteID_ and _Password_ are different credentials than the ones used to log in to a DHL account.
...
Then, in Odoo, on the shipping method form, enter the _DHL API Key_ in the DHL SiteID field, and the _API Secret_ in the DHL Password field.
...
If a DHL Express account is **not** available:
  ...
  4. After setting up the DHL Express account, go here to get the _DHL API Key_ and _API Secret_ credentials.
