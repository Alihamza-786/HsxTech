...
In Odoo, _quality control points_ (QCPs), are used to automatically create quality checks at predetermined intervals. QCPs can be configured to create quality checks for specific operations (manufacturing, delivery, etc.), as well as specific products within those operations.
