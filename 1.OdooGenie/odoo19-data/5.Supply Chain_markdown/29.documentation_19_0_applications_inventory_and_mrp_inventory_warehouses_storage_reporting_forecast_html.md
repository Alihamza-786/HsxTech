...
The **Inventory** _forecasted report_ provides a real-time view of projected stock levels, helping businesses manage their inventory efficiently. This report is beneficial for planning and decision making, ensuring stock availability for upcoming sales, manufacturing, and replenishment activities.

Important

The forecast report is **only** available on products where inventory is being tracked, commonly referred to as a _storable product_.
...
To access the report, click the __ Forecasted smart button on a product form. Alternatively, the report can be accessed from a sales order (SO) or purchase order (PO) by clicking on the __(Graph) icon next to the product, then selecting __ View Forecast.

The forecasted report consists of a graph and a table. The graph visually represents stock movements over time. The following information is displayed:
...
The table provides detailed metrics regarding operations, including:
  ...
  * Forecasted with Pending: The updated stock levels with the pending stock movements considered.
...
The forecasted quantity for today shows `-5`, because the confirmed demand is five, but the current stock on hand is zero. Since the Receipt date is _after_ the Delivery Date, the shipment is late.
