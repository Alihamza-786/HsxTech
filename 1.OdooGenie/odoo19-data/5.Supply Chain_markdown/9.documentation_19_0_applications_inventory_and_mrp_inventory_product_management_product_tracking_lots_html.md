Source: https://www.odoo.com/documentation/19.0/applications/inventory_and_mrp/inventory/product_management/product_tracking/lots.html
...
On the product form, click into the General Information tab. In the Track Inventory field, tick the checkbox, then select By Lots from the drop-down menu. Now, new or existing lot numbers can be assigned to newly-received or manufactured batches of this product.
...
After a new lot number has been created, saved, and assigned to the desired product, the lot number is saved as an existing lot number linked to the product, and can be selected when assigning lot numbers to products on a receipt.
