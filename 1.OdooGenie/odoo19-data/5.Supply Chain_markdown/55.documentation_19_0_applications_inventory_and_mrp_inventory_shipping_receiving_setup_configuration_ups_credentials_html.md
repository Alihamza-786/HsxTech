Source: https://www.odoo.com/documentation/19.0/applications/inventory_and_mrp/inventory/shipping_receiving/setup_configuration/ups_credentials.html
...
Next, log into the UPS developer account to generate the developer key. To begin, click the profile icon in the top-right corner, and choose the Apps option from the drop-down menu.
...
In the UPS Configuration tab, complete the following fields:
  ...
  * UPS Package Type: (_required_) Select from the drop-down menu the package type that is supported for the shipping service.
