Source: https://www.odoo.com/documentation/19.0/applications/essentials/reporting.html
...
After choosing what you want to measure, you can define how the data should be grouped depending on the dimension you want to analyze. By default, the data is often grouped by _Date > Month_, which is used to analyze the evolution of a measure over the months.
...
Grouping data is quintessential to the pivot view. It enables drilling down the data to gain deeper insights. While you can use the Group By option to quickly add a group at the level of rows, as shown in the example above, you can also click the plus button (➕) next to the Total header at the level of rows _and_ columns, and then select one of the **preconfigured groups**. To remove one, click the minus button (➖).
